...
CREATE VIEW CONTENT_VIEW AS
    ...
    FROM Content A JOIN ContentType B JOIN ContentProvider C #JOIN Cast D
    ON A.ContentTypeId = B.Id AND A.contentProviderId = C.id; #AND A.id = D.contentId;
...
CREATE VIEW ACTORS_VIEW AS
	SELECT A.id as contentId, group_concat(concat(C.fname, ' ', C.lname) separator ',') as actors
    FROM Content A JOIN Cast B JOIN Actor C
    ON A.id = B.contentId AND B.actorId = C.id
    GROUP BY A.id;

DELIMITER //
CREATE TRIGGER after_viewer_watched BEFORE INSERT 
ON Watches 
FOR EACH ROW 
	BEGIN
		SET New._timestamp = NOW();
END //

DELIMITER ;;
CREATE TRIGGER update_viewer_watched before UPDATE ON Watches FOR EACH ROW 
	BEGIN
		SET new._timestamp = now();
    END;;